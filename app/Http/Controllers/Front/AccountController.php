...
class AccountController extends Controller
{
    ...
    public function dashboard(Request $request): View
    {
        ...
        $fields = [
            ...
            trim((string) ($profile?->company ?? '')),
            trim((string) ($user->email ?? '')),
        ];
        $filled = collect($fields)->filter(static fn (string $value): bool => $value !== '')->count();
        $profileCompletion = (int) round(($filled / max(1, count($fields))) * 100);
