...
class TaxController extends Controller
{
    ...
    public function show(Request $request): View
    {
        ...
        $taxCategory = $this->resolveConfiguredBlogCategory(
            (array) ($pagePayload['blog_source'] ?? []),
            $locale,
            $fallbackLocale
        );
        $categoryTranslation = $taxCategory?->translations->firstWhere('locale', $locale)
            ?? $taxCategory?->translations->firstWhere('locale', $fallbackLocale)
            ?? $taxCategory?->translations->first();
        $categorySlug = trim((string) ($categoryTranslation?->slug ?? ''));
        $defaultCategoryName = str_starts_with(strtolower($locale), 'hr') ? 'Porezi' : 'Tax';
        $categoryName = trim((string) ($categoryTranslation?->name ?? '')) ?: $defaultCategoryName;
        $taxPosts = $this->resolveTaxPosts(
            (array) ($pagePayload['blog_source'] ?? []),
            $taxCategory,
            $locale,
            $fallbackLocale
        ...
        return view($this->frontendView($request, 'pages.tax'), [
            'taxPosts' => $taxPosts,
            'taxCategoryName' => $categoryName,
            'taxArchiveUrl' => $categorySlug !== ''
            ...
            'complianceSection' => (array) ($translationPayload['compliance'] ?? []),
            'reviewSection' => (array) ($translationPayload['review'] ?? []),
            'optimizationSection' => (array) ($translationPayload['optimization'] ?? []),
            'dueDiligenceSection' => (array) ($translationPayload['due_diligence'] ?? []),
            'transferPricingSection' => (array) ($translationPayload['transfer_pricing'] ?? []),
            'meetingSection' => (array) ($translationPayload['meeting'] ?? []),
            'blogSection' => $blogSection,
            'heroBackgroundUrl' => $this->resolveServiceHeroBackgroundUrl($servicePage),
            'taxIllustrationUrl' => $this->versionedAsset('front-theme/images/services/tax-editorial-3d.svg'),
            'servicePageTitle' => trim((string) ($servicePageTranslation?->title ?? '')) ?: 'Porezi',
    ...
    private function resolveServicePage(string $locale, string $fallbackLocale): array
    {
        ...
        $servicePage = ServicePage::query()
            ...
            ->orderByRaw('case when code = ? then 0 else 1 end', [ServicePageTemplateRegistry::defaultCode(ServicePageTemplateRegistry::TAX)])
            ->orderBy('sort_order')
            ->orderBy('id')
            ->first();
    ...
    private function resolveConfiguredBlogCategory(array $blogSource, string $locale, string $fallbackLocale): ?Category
    {
        ...
        return $this->resolveTaxCategory($locale, $fallbackLocale);
    }

    /**
     * @return Collection<int, BlogPost>
     */
    private function resolveTaxPosts(
        array $blogSource,
    ...
    private function resolveTaxCategory(string $locale, string $fallbackLocale): ?Category
    {
        return Category::query()
            ...
            ->map(fn (Category $category): array => [
                'category' => $category,
                'score' => $this->taxCategoryScore($category),
            ])
    ...
    private function taxCategoryScore(Category $category): int
    {
        $slugCandidates = ['tax', 'porezi', 'porez'];
        $nameCandidates = ['tax', 'taxes', 'porezi', 'porez'];
        $bestScore = 100;
        ...
        foreach ($category->translations as $translation) {
            ...
            if (str_contains($slug, 'porez') || str_contains($slug, 'tax')) {
                $bestScore = min($bestScore, 2);
                continue;
            }

            if (str_contains($name, 'porez') || str_contains($name, 'tax')) {
    ...
    private function resolveServiceHeroBackgroundUrl(?ServicePage $servicePage): string
    {
        ...
        return $this->versionedAsset('front-theme/images/services/tax-editorial-3d.svg');
    }
