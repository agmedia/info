...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init_template(){
        ...
        // Attaching Menu Hider (safe for SPA/Inertia)
...
// Reset safely (old code chained classList after remove() which can throw)
        setTimeout(function(){
            var h = document.querySelector('.menu-hider');
            if(h){ h.classList.remove('menu-active'); }
        }, 50);
    ...
    function waitForPageAndInit(attempts = 0){
        ...
        requestAnimationFrame(() => waitForPageAndInit(attempts + 1));
    }

// expose (korisno ako kasnije želiš ručno re-init nakon Inertia navigacije)
    window.__appkit_init = waitForPageAndInit;
