<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ __('Content / Calls') }}</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{{ $isEdit ? __('Edit Call Post') : __('Create Call Post') }}</h1>
                <p class="mt-2 text-sm text-slate-600">{{ __('Core call data, locale content and SEO.') }}</p>
            </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        @if ($activeTab === 'content')
            <div class="admin-panel admin-form-panel p-6">
                ...
                <div class="mt-3 grid gap-3" style="grid-template-columns: repeat(12, minmax(0, 1fr));">
                    ...
                    <div class="sm:col-span-7 flex items-end gap-3" style="grid-column: span 7;">
                        <button
                            ...
                            aria-label="{{ __('Toggle call post active state') }}"
                        >
                ...
                <div class="mt-4" wire:key="call-post-body-{{ $postId ?? 'new' }}-{{ $form['locale'] }}">
                    <label for="call-post-body-html" class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('Body') }}</label>
                    <textarea id="call-post-body-html" rows="14" wire:model.live.debounce.300ms="form.body_html" data-quill-editor data-quill-image-upload-url="{{ route('admin.content.calls.editor-image.upload') }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></textarea>
        ...
        @if ($activeTab === 'media')
            <livewire:admin.media.manager
                :model-class="\App\Models\Content\Call\CallPost::class"
                :model-id="$postId"
                :locale="$form['locale']"
                :wire:key="'call-post-media-manager-'.($postId ?? 'new').'-'.$form['locale']"
            />
        @endif
