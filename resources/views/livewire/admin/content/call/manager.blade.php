<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold tracking-tight">{{ __('Call Posts') }}</h1>
                <p class="mt-1 text-sm text-slate-600">{{ __('Manage EU fund calls with locale-specific slugs, categories, and SEO fields.') }}</p>
            ...
            <div class="flex w-[64rem] max-w-full items-end justify-end gap-3">
                ...
                <a href="{{ route('admin.content.calls.create', ['locale' => $locale]) }}" class="rounded-xl bg-cyan-700 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-800">
                    {{ __('Create') }}
                </a>
            </div>
        </div>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="admin-items-table min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('admin.content.calls.edit', ['callPost' => $row->id, 'locale' => $locale]) }}" class="rounded-lg border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                                        {{ __('Edit') }}
                                    </a>
                                    <button
                                        type="button"
                                        wire:click="delete({{ $row->id }})"
                                        wire:confirm="{{ __('Delete this call post?') }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-3 py-8 text-center text-sm text-slate-500">{{ __('No call posts yet.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
