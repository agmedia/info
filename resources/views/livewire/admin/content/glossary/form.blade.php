<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ __('admin.content.glossary.form.eyebrow') }}</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{{ $isEdit ? __('admin.content.glossary.form.edit_title') : __('admin.content.glossary.form.create_title') }}</h1>
                <p class="mt-2 text-sm text-slate-600">{{ __('admin.content.glossary.form.subtitle') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <span class="admin-chip">{{ __('Locale:') }} {{ $form['locale'] }}</span>
                <button type="button" wire:click="backToList" class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">{{ __('admin.content.glossary.form.buttons.back') }}</button>
            </div>
        </div>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="admin-panel admin-form-panel p-6">
            <p class="admin-section-title">{{ __('admin.content.glossary.form.sections.core') }}</p>
            ...
            <div class="mt-4 grid gap-3" style="grid-template-columns: repeat(12, minmax(0, 1fr));">
                ...
                <div style="grid-column: span 3;">
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin.content.glossary.form.fields.collection') }}</label>
                    <select wire:model.live="collectionCodeSelection" data-tom-select data-tom-no-search="1" class="admin-select w-full rounded-xl border border-slate-300 px-3 py-2 text-sm lowercase">
                        @foreach ($glossaryCollectionOptions as $collectionCode)
                            <option value="{{ $collectionCode }}">{{ $collectionCode }}</option>
                        @endforeach
                        <option value="{{ $customCollectionOptionValue }}">{{ __('admin.content.glossary.form.fields.new_collection') }}</option>
                    </select>
                    @if ($collectionCodeSelection === $customCollectionOptionValue)
                        <input
                            type="text"
                            wire:model.live.debounce.300ms="customCollectionCode"
                            class="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm font-mono lowercase"
                            placeholder="{{ __('admin.content.glossary.form.fields.new_collection_placeholder') }}"
                        />
                    @endif
                    @error('form.collection_code') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <button
                    ...
                    aria-label="{{ __('admin.content.glossary.form.fields.toggle_active') }}"
                >
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="admin-panel admin-form-panel p-6">
                <p class="admin-section-title">{{ __('admin.content.glossary.form.sections.content') }}</p>

                <div class="mt-4">
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin.content.glossary.form.fields.title') }}</label>
                ...
                <div class="mt-3">
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin.content.glossary.form.fields.excerpt') }}</label>
                    <textarea rows="3" wire:model="form.excerpt" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></textarea>
                </div>

                <div class="mt-3" wire:key="glossary-body-{{ $termId ?? 'new' }}-{{ $form['locale'] }}">
                    <label for="glossary-body-html" class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin.content.glossary.form.fields.body') }}</label>
                    <textarea id="glossary-body-html" rows="10" wire:model.live.debounce.300ms="form.body_html" data-quill-editor class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></textarea>
                    @error('form.body_html') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="admin-panel admin-form-panel p-6">
                <p class="admin-section-title">{{ __('admin.content.glossary.form.sections.seo') }}</p>
                ...
                <div class="mt-3">
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin.content.glossary.form.fields.payload') }}</label>
        ...
        <div class="admin-form-actions flex items-center gap-2 pt-2">
            <button type="submit" class="rounded-xl bg-cyan-700 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-800">
                {{ $isEdit ? __('admin.content.glossary.form.buttons.update') : __('admin.content.glossary.form.buttons.create') }}
            </button>
            <button type="button" wire:click="backToList" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                {{ __('admin.content.glossary.form.buttons.cancel') }}
            </button>
        </div>
    </form>
</div>
