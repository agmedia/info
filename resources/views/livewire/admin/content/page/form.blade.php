<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ in_array(($form['layout'] ?? ''), ['about', 'career'], true) ? 'Sadržaj / Stranice' : __('Content / Pages') }}</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-900">
                    @if (($form['layout'] ?? '') === 'about')
                        Uredi stranicu: O nama
                    @elseif (($form['layout'] ?? '') === 'career')
                        Uredi stranicu: Ljudski potencijali
                    @else
                        {{ $isEdit ? __('Edit Info Page') : __('Create Info Page') }}
                    @endif
                </h1>
                <p class="mt-2 text-sm text-slate-600">
                    {{ in_array(($form['layout'] ?? ''), ['about', 'career'], true) ? 'Uredite samo sadržaj koji se prikazuje na ovoj konkretnoj stranici.' : __('Static page structure, locale content and SEO.') }}
                </p>
            </div>
    ...
    <form wire:submit="save" class="space-y-6" data-admin-dirty-form>
        ...
        <div class="admin-panel admin-form-panel p-3 sm:p-4">
            <div class="flex flex-wrap gap-2">
                ...
                @endif
                @if (in_array(($form['layout'] ?? ''), ['academy', 'career', 'references'], true))
                    <button type="button" wire:click="setTab('media')" class="rounded-lg border px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] {{ $activeTab === 'media' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-100' }}">
                        {{ ($form['layout'] ?? '') === 'career' ? 'Galerija' : __('Media') }}
                    </button>
                @endif
        ...
        @if ($activeTab === 'content')
            @if (($form['layout'] ?? '') === 'about')
                @include('livewire.admin.content.page.partials.about-editor')
            @elseif (($form['layout'] ?? '') === 'career')
                @include('livewire.admin.content.page.partials.career-editor')
            @else
                ...
                <div class="mt-4 grid gap-3 md:grid-cols-2">
                    @unless ($isEdit)
                        <div>
                            ...
                            <select wire:model.live="form.layout" data-tom-select data-tom-no-search="1" class="admin-select w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                                <option value="default">Standardna stranica</option>
        ...
        @include('livewire.admin.partials.form-actions', [
            'previewUrl' => $isEdit && trim((string) ($form['slug'] ?? '')) !== '' ? route('pages.show', ['slug' => $form['slug']]) : null,
            'submitLabel' => $isEdit ? __('Spremi stranicu') : __('Kreiraj stranicu'),
        ])
    </form>
</div>
