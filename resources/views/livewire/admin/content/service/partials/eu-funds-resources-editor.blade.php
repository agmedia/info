<div id="eu-funds-resources-admin" class="admin-panel admin-form-panel scroll-mt-24 p-6">
    ...
    <div class="mt-6 space-y-5">
        @foreach (($translationPayload['resources']['cards'] ?? []) as $cardIndex => $card)
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                <div class="flex items-center justify-between gap-3"><p class="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-700">Kartica programa {{ $cardIndex + 1 }}</p><button type="button" wire:click="removeTranslationListItem('resources.cards', {{ $cardIndex }})" class="text-xs font-semibold text-rose-600 hover:text-rose-700">Ukloni</button></div>
    ...
    <button type="button" wire:click="addTranslationListItem('resources.cards', 'eu_funds_resource_card')" class="mt-4 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Dodaj karticu programa</button>
</div>
