    ...
    <div class="admin-panel admin-form-panel p-6">
        <div class="mb-4 flex flex-wrap gap-2">
            @foreach ([
                ...
                @continue($tabKey === 'newsletter' && ! $canManageNewsletter)
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            @if ($tab === 'newsletter' && $canManageNewsletter)
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="md:col-span-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div>
                                <h3 class="text-sm font-bold text-slate-800">{{ __('admin.settings.store.newsletter.title') }}</h3>
                                <p class="mt-1 text-xs text-slate-600">{{ __('admin.settings.store.newsletter.description') }}</p>
                            </div>
                            @if (
                                \Illuminate\Support\Facades\Route::has('admin.messages.newsletter.index')
                                && auth()->user()
                                && (auth()->user()->isA('superadmin') || auth()->user()->can('messages.newsletter.view'))
                            )
                                <a href="{{ route('admin.messages.newsletter.index') }}" wire:navigate class="rounded-xl border border-cyan-700 px-3 py-2 text-xs font-semibold text-cyan-800 hover:bg-cyan-50">
                                    {{ __('admin.settings.store.newsletter.open_subscriptions') }}
                                </a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2 md:max-w-sm">
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500" for="store-newsletter-provider">{{ __('admin.settings.store.newsletter.provider') }}</label>
                        <select id="store-newsletter-provider" wire:model.live="form.store_newsletter_provider" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                            <option value="none">{{ __('admin.settings.store.newsletter.provider_none') }}</option>
                            <option value="mailchimp">{{ __('admin.settings.store.newsletter.provider_mailchimp') }}</option>
                        </select>
                        @error('form.store_newsletter_provider') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                    </div>

                    @if (($form['store_newsletter_provider'] ?? 'none') === 'none')
                        <div class="md:col-span-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                            {{ __('admin.settings.store.newsletter.disabled_help') }}
                        </div>
                    @endif

                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500" for="store-newsletter-mailchimp-server-prefix">{{ __('admin.settings.store.newsletter.server_prefix') }}</label>
                        <input id="store-newsletter-mailchimp-server-prefix" type="text" wire:model="form.store_newsletter_mailchimp_server_prefix" placeholder="us21" autocomplete="off" spellcheck="false" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" />
                        <p class="mt-1 text-xs text-slate-500">{{ __('admin.settings.store.newsletter.server_prefix_help') }}</p>
                        @error('form.store_newsletter_mailchimp_server_prefix') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500" for="store-newsletter-mailchimp-list-id">{{ __('admin.settings.store.newsletter.list_id') }}</label>
                        <input id="store-newsletter-mailchimp-list-id" type="text" wire:model="form.store_newsletter_mailchimp_list_id" autocomplete="off" spellcheck="false" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" />
                        <p class="mt-1 text-xs text-slate-500">{{ __('admin.settings.store.newsletter.list_id_help') }}</p>
                        @error('form.store_newsletter_mailchimp_list_id') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="md:col-span-2 rounded-xl border border-slate-200 p-4">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <label class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500" for="store-newsletter-mailchimp-api-key">{{ __('admin.settings.store.newsletter.api_key') }}</label>
                            <span class="rounded-full px-2 py-1 text-xs font-semibold {{ $hasMailchimpApiKey ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600' }}">
                                {{ $hasMailchimpApiKey ? __('admin.settings.store.newsletter.api_key_saved') : __('admin.settings.store.newsletter.api_key_missing') }}
                            </span>
                        </div>
                        <input id="store-newsletter-mailchimp-api-key" type="password" wire:model="mailchimpApiKey" placeholder="{{ $hasMailchimpApiKey ? '••••••••••••••••••••••••••••••••-'.(($form['store_newsletter_mailchimp_server_prefix'] ?? '') ?: 'usXX') : '' }}" autocomplete="new-password" spellcheck="false" @disabled($clearMailchimpApiKey) class="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-100" />
                        <p class="mt-1 text-xs text-slate-500">
                            {{ $hasMailchimpApiKey ? __('admin.settings.store.newsletter.api_key_preserve_help') : __('admin.settings.store.newsletter.api_key_new_help') }}
                        </p>
                        @error('mailchimpApiKey') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror

                        @if ($hasMailchimpApiKey)
                            <label class="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-rose-700">
                                <input type="checkbox" wire:model.live="clearMailchimpApiKey" class="rounded border-slate-300 text-rose-700 focus:ring-rose-500" />
                                {{ __('admin.settings.store.newsletter.clear_api_key') }}
                            </label>
                            <p class="mt-1 text-xs text-slate-500">{{ __('admin.settings.store.newsletter.clear_api_key_help') }}</p>
                        @endif
                    </div>
                </div>
            @endif
